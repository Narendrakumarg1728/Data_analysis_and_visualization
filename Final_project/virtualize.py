...
for i in range(8, sheet1.nrows):                              """Data Wrangling to get the data from SurvivalByStage.xlsx"""
...
def color_filler(x):                                                                         """Function to select different colors depending on the survival rate and mortality"""
    y = []
...
factors = ["Brain and other nervous system", "Breast", "Cervix", "Colorectum", "Esophagus", "Hodgkin lymphoma", "Kidney and renal pelvis","Larynx", "Liver and intrahepatic bile duct", "Lung and bronchus", "Melanoma of the skin","Myeloma", "Non-Hodgkin lymphoma", "Oral cavity and pharynx", "Ovary","Pancreas","Prostate", "Stomach", "Testis", "Thyroid", "Urinary bladder", "Uterine corpus"]    """Different cancer types"""
...
hover1 = HoverTool(
        tooltips=[
                 ("Cancer Type",  "$y"),
                     ("Survival rate",  "$x{int}")])
s1 = figure(y_range=factors,tools=[hover1],plot_width=800, plot_height=800,title="5 Years Relative Survival Rate", title_location="above", toolbar_location=None)
col = color_filler(x)
